...
#problem clusters with high MAE over 6 will train per station within these clusters
high_error_clusters = [5, 6, 13]  
...
stations_in_high_clusters = [station_id for station_id, cluster in time_cluster_map.items()
                             if cluster in high_error_clusters]
...
# Optional: show a few affected station numbers
    ...
    #Cluster
    kmeans_time = KMeans(n_clusters=15, random_state=42)
...
os.makedirs("models_per_station", exist_ok=True)

print("\n--- Training per-station models for high-error clusters ---")

for station_id in stations_in_high_clusters:
    station_df = merged_df[merged_df['number'] == station_id].copy()

    if len(station_df) < 2000:
        print(f"Skipping station {station_id} — not enough data.")
        continue

    # Reuse your existing feature list
    ...
    mae_station = mean_absolute_error(y_test, preds)

    print(f"Station {station_id} model trained — MAE: {mae_station:.2f}")

    joblib.dump({'model': model, 'features': features},
                f"models_per_station/station_{station_id}.pkl")
