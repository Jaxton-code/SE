...
function displayStationDetail(station) {
    ...
    $('#stationDetail').html(`
        ...
        <p><strong>Bikes Available:</strong> <span class="bikes-count ${station.available_bikes <= 3 ? 'low-availability' : ''}">${station.available_bikes}</span></p>
        <p><strong>Stands Available:</strong> <span class="stands-count">${station.available_bike_stands}</span></p>
        <p><strong>Status:</strong> ${station.status}</p>
        <p><strong>Location:</strong> (${station.position_lat}, ${station.position_lng})</p>

        <div class="predict-container">
            <p><strong>Select Future Time:</strong></p>
            <input type="datetime-local" class="date-input" id="predictTime">
            <button id="predictBtn" class="predict-btn">Predict Availability</button>
...
function displayNearestStationDetail(station ,distance) {
    $('#stationDetail').html(`
        <div class="station-detail-card">
            <h3>Your nearest Bike Station:<br> ${station.name}</br> </h3>
            <p><strong>Address:</strong> ${station.address}</p>
...
async function loadTrendChart(stationName) {
    ...
    try {
        ...
        // Create new chart with custom time formatting
            ...
            options: {
                ...
                scales: {
                    ...
                    x: {
                        ...
                        ticks: {
                            maxTicksLimit: 10,
                            // Add custom callback to format dates
                            callback: function(value, index, values) {
                                // Parse the date string and format as MM-DD HH:MM
